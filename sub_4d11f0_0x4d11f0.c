// 函数名称: sub_4d11f0
// 虚拟地址: 0x4d11f0
// 源二进制: E:/torrent/Cursor/Roll for the Galaxy/Roll.exe.bndb

int32_t* __fastcallsub_4d11f0(int32_t arg1, int32_t arg2)
{
    // 第一条实际指令: int32_t var_8 = 0xffffffff
    int32_t var_8 = 0xffffffff
    int32_t (* var_c)(void* arg1) = sub_59fe40
    TEB* fsbase
    struct _EXCEPTION_REGISTRATION_RECORD* ExceptionList = fsbase->NtTib.ExceptionList
    int32_t __saved_ebp
    int32_t var_24 = __security_cookie ^ &__saved_ebp
    fsbase->NtTib.ExceptionList = &ExceptionList
    void* eax_3 = data_114e844
    *(eax_3 + 0x1c) += 1
    int32_t var_28 = *(eax_3 + 0x1c)
    char* var_18
    sub_48a9d0(&var_18, "!Mem\%d")
    int32_t var_8_1 = 0
    char* eax_4 = var_18
    char* edx = &data_5b2591
    
    if (eax_4 != 0)
        edx = eax_4
    
    char* const var_14
    sub_4d18a0(eax_4, edx, &var_14, arg1)
    var_8_1.b = 1
    char* const ecx_1 = &data_5b2591
    char* const eax_5 = var_14
    
    if (eax_5 != 0)
        ecx_1 = eax_5
    
    int32_t* result = sub_4d0b50(ecx_1, arg1)
    
    if (*result != 0)
        sub_489550(result, &data_5b2591, "pAsset->pAssetData == NULL", 
            "d:\ax\trunk\ax2017\engine\assetcatalog.cpp", 0x13e, "AssetCatalogNewMemoryAsset")
        
        if (IsDebuggerPresent() == 1)
            breakpoint
        
        sub_489700()
        noreturn
    
    *result = arg2
    result[2] = 3
    sub_48a5e0(&result[3], &data_5b2591)
    var_8_1.b = 2
    
    if (data_aca1f4 != 0)
        char* eax_6 = var_14
        
        if (eax_6 != 0 && *eax_6 != 0)
            char* eax_7 = sub_48a080(&var_14)
            int32_t temp0_1 = *(eax_7 + 4)
            *(eax_7 + 4) -= 1
            
            if (temp0_1 == 1)
                sub_4984f0(eax_7, *(eax_7 + 0xc) + 0x10)
                var_14 = &data_5b2591
    
    int32_t var_8_2 = 3
    
    if (data_aca1f4 != 0)
        char* eax_8 = var_18
        
        if (eax_8 != 0 && *eax_8 != 0)
            char* eax_9 = sub_48a080(&var_18)
            int32_t temp1_1 = *(eax_9 + 4)
            *(eax_9 + 4) -= 1
            
            if (temp1_1 == 1)
                sub_4984f0(eax_9, *(eax_9 + 0xc) + 0x10)
    
    fsbase->NtTib.ExceptionList = ExceptionList
    return result
}
