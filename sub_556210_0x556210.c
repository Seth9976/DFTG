...
LRESULT __convention("regparm")sub_556210(int32_t arg1, int32_t arg2, HWND arg3, int16_t arg4)
{
    // 第一条实际指令: int32_t __saved_ebp
    int32_t __saved_ebp
    int32_t eax_1 = __security_cookie ^ &__saved_ebp
    void* esi = data_1511868
    HWND hWnd_2 = arg3
    HWND hWnd = hWnd_2
    LRESULT result
    ...
    if (hWnd_1 != 0)
            ...
            if (eax_6 != 0xffffffff && eax_6 != 0)
                ...
                if (ecx_6 == 5)
                    *(eax_6 + 0x10)
                    ...
                    sub_553d20(&var_5c, 0x50, "%s %d")
                else if (ecx_6 != 8)
                    __mbsnbcmp(&var_5c, *(eax_6 + 0x10), 0x4f)
        ...
        void var_5b
        result = __mbscat_s(&var_ac, &var_5c, ecx_8 - &var_5b)
        
        if (result != 0)
            _memset(&var_ac, 0, 0x4f)
            __mbsnbcmp(&var_ac, &var_5c, 0x4f)
            result = SendMessageA(*(data_1511868 + 0xc), 0x40c, 0, &lParam)
