...
BOOL __convention("regparm")?write_requires_double_translation_nolock@@YA_NH@Z(int32_t arg1, int32_t arg2, enum CONSOLE_MODE arg3, int32_t arg4)
{
    ...
    if (__isatty(arg4) != 0)
        ...
        if (*((&data_6cfb08)[edi_2] + esi_2 + 0x28) s< 0)
            eax_1 = sub_58c1b2()[0x13]
        
        if (*((&data_6cfb08)[edi_2] + esi_2 + 0x28) s< 0
                && (*(eax_1 + 0xa8) != 0 || *((&data_6cfb08)[edi_2] + esi_2 + 0x29) != 0))
            result.b = GetConsoleMode(*((&data_6cfb08)[edi_2] + esi_2 + 0x18), &mode) != 0
        else
            result.b = 0
    else
        result.b = 0
